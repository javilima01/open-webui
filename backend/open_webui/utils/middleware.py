...
async def process_chat_response(
    ...
    if not isinstance(response, StreamingResponse):
        if event_emitter:
            ...
            if is_api_request and isinstance(response, dict):
                choices = response.get("choices", [])
                if choices and choices[0].get("message", {}).get("content"):
                    content = choices[0]["message"]["content"]
                    if "event" in content:
                        final_message = None
                        for match in re.finditer(r"\{.*?event.*?\}", content):
                            try:
                                event_obj = ast.literal_eval(match.group(0))
                                event = event_obj.get("event", {})
                                if event.get("type") == "message":
                                    final_message = event.get("data", {}).get("content")
                            except Exception:
                                pass
                        if final_message:
